# Copy database schema from production to sandbox
param(
    [string]$ResourceGroupName = "NWIApps",
    [string]$ServerName = "nwiapps",
    [string]$SourceDatabase = "sqldb-steel-estimation-prod",
    [string]$TargetDatabase = "sqldb-steel-estimation-sandbox"
)

Write-Host "Database Schema Copy Script" -ForegroundColor Cyan
Write-Host "This will copy the schema (tables, not data) from production to sandbox" -ForegroundColor Yellow

# Get SQL Server admin credentials
Write-Host "`nPlease enter SQL Server admin credentials:" -ForegroundColor Yellow
$credential = Get-Credential -Message "Enter SQL admin username and password"
$sqlUsername = $credential.UserName
$sqlPassword = $credential.GetNetworkCredential().Password

# Connection strings
$sourceConnString = "Server=$ServerName.database.windows.net;Database=$SourceDatabase;User Id=$sqlUsername;Password=$sqlPassword;Encrypt=True;TrustServerCertificate=False;"
$targetConnString = "Server=$ServerName.database.windows.net;Database=$TargetDatabase;User Id=$sqlUsername;Password=$sqlPassword;Encrypt=True;TrustServerCertificate=False;"

Write-Host "`nGenerating schema script from production database..." -ForegroundColor Yellow

# SQL script to get all table creation scripts
$schemaScript = @"
-- Create tables script will be generated here
-- For now, let's create the essential tables manually
...
try {
    Write-Host "Connecting to sandbox database..." -ForegroundColor Yellow
    
    # Execute the schema script on sandbox database
    Invoke-Sqlcmd -ConnectionString $targetConnString -Query $schemaScript -ErrorAction Stop
    
    Write-Host "`nSchema copied successfully to sandbox database!" -ForegroundColor Green
...
"@
    ...
    Invoke-Sqlcmd -ConnectionString $targetConnString -Query $createUserScript
    
    Write-Host "`n========================================" -ForegroundColor Cyan
    Write-Host "Sandbox Database Setup Complete!" -ForegroundColor Green
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host "Database: $TargetDatabase" -ForegroundColor White
    Write-Host "Schema: Created successfully" -ForegroundColor White
    Write-Host "Admin User: admin (Password: Admin123!)" -ForegroundColor Yellow
    Write-Host "========================================" -ForegroundColor Cyan
    
} catch {
    Write-Error "Failed to copy schema: $_"
    Write-Host "`nTroubleshooting tips:" -ForegroundColor Yellow
    Write-Host "1. Make sure you have the correct SQL admin credentials" -ForegroundColor Gray
    Write-Host "2. Ensure the sandbox database was created" -ForegroundColor Gray
    Write-Host "3. Check firewall rules allow your IP" -ForegroundColor Gray
    Write-Host "4. Try using SQL Server Management Studio instead" -ForegroundColor Gray
}