-- =====================================================
-- SAFE Multi-Tenant & Organization Migration Script
-- =====================================================
-- This script checks for table existence before running
-- Run this script to add company/organization support to your database

PRINT '========================================='
PRINT 'Starting Safe Multi-Tenant Migration'
...
BEGIN
    ...
    PRINT 'This migration requires an existing database with Users table.'
    PRINT 'Please ensure you have run the initial Entity Framework migrations first.'
    RETURN
END
ELSE
BEGIN
...
BEGIN
    PRINT 'ERROR: Email column does not exist in Users table!'
    PRINT 'This migration requires the standard Users table structure.'
    RETURN
END
ELSE
BEGIN
    PRINT '✓ Email column exists in Users table'
END
...
IF EXISTS (SELECT 1 FROM [dbo].[Users] WHERE Email = 'admin@steelestimation.com')
BEGIN
    ...
    ELSE
        PRINT '! Admin user already has a company assigned'
END
ELSE
BEGIN
    PRINT '! Admin user (admin@steelestimation.com) not found'
END
...
BEGIN
    ...
    FROM [dbo].[Users] u
    LEFT JOIN [dbo].[Companies] c ON u.CompanyId = c.Id
...
DECLARE @TotalCompanies INT = (SELECT COUNT(*) FROM Companies)
DECLARE @UsersWithCompany INT = (SELECT COUNT(*) FROM [dbo].[Users] WHERE CompanyId IS NOT NULL)
DECLARE @UsersWithoutCompany INT = (SELECT COUNT(*) FROM [dbo].[Users] WHERE CompanyId IS NULL)
DECLARE @MaterialTypes INT = (SELECT COUNT(*) FROM CompanyMaterialTypes)
DECLARE @MbeIdMappings INT = (SELECT COUNT(*) FROM CompanyMbeIdMappings)
DECLARE @MaterialPatterns INT = (SELECT COUNT(*) FROM CompanyMaterialPatterns)

PRINT 'Total Companies: ' + CAST(@TotalCompanies AS VARCHAR)
PRINT 'Users with Company: ' + CAST(@UsersWithCompany AS VARCHAR)
PRINT 'Users without Company: ' + CAST(@UsersWithoutCompany AS VARCHAR)
PRINT 'Material Types: ' + CAST(@MaterialTypes AS VARCHAR)
PRINT 'MBE ID Mappings: ' + CAST(@MbeIdMappings AS VARCHAR)
PRINT 'Material Patterns: ' + CAST(@MaterialPatterns AS VARCHAR)
...
SELECT TOP 5
    ...
    c.MaxUsers
FROM Companies c
LEFT JOIN [dbo].[Users] u ON c.Id = u.CompanyId
