# Simple Azure SQL Setup - Minimal approach that works
Write-Host "=== Simple Azure SQL Setup ===" -ForegroundColor Cyan
...
Write-Host "Creating tables and data using minimal commands..." -ForegroundColor Green

# Use a single large script file approach
$setupScript = @"
...
$setupScript | Out-File -FilePath "azure-simple-setup.sql" -Encoding UTF8

Write-Host "Executing setup script..." -ForegroundColor Yellow

try {
    # Try a simple approach - execute the SQL file directly
    docker run --rm -v ${PWD}:/scripts mcr.microsoft.com/mssql-tools:latest /opt/mssql-tools/bin/sqlcmd -S $azureServer -d $azureDatabase -U $azureUsername -P $azurePassword -i /scripts/azure-simple-setup.sql -I
    
    Write-Host "Setup script executed" -ForegroundColor Green
    
    # Quick verification
    Write-Host "`nVerifying setup..." -ForegroundColor Cyan
    $verification = docker run --rm mcr.microsoft.com/mssql-tools:latest /opt/mssql-tools/bin/sqlcmd -S $azureServer -d $azureDatabase -U $azureUsername -P $azurePassword -Q "SELECT COUNT(*) as TableCount FROM sys.tables WHERE is_ms_shipped = 0" -h -1
    
    Write-Host "Tables created: $($verification.Trim())" -ForegroundColor Green
    
    # Check data
    $dataCheck = docker run --rm mcr.microsoft.com/mssql-tools:latest /opt/mssql-tools/bin/sqlcmd -S $azureServer -d $azureDatabase -U $azureUsername -P $azurePassword -Q "SELECT 'Users: ' + CAST(COUNT(*) AS VARCHAR) FROM AspNetUsers" -h -1
    
    Write-Host "Admin user created: $($dataCheck.Trim())" -ForegroundColor Green
    
    Write-Host "`n✅ Azure SQL setup completed!" -ForegroundColor Green
    Write-Host "Login: admin@steelestimation.com / Admin@123" -ForegroundColor Cyan
